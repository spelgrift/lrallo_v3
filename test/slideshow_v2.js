(function($) {
	...
	var slideMan = function(element, options)
	{
		...
		{
			var _speed = $slideshow.attr('data-sm-speed'),
			_duration = $slideshow.attr('data-sm-duration');
			if($slideshow.hasClass('sm-auto')) {
				settings.autoSlideshow = true;
			}
			if($slideshow.hasClass('sm-fade')) {
			...
			if(typeof _duration !== typeof undefined && _duration !== false) {
				settings.slideDuration = _duration;
			}
		}
		obj.updateSettings();
